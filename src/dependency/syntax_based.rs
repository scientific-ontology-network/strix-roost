...
pub trait SyntaxBasedDependency<T:ForIRI>: DependencyBuilder<T> {
    ...
    fn dependency_from_subsumption(sco: &SubClassOf<T>) -> HashSet<(Term<T>, Term<T>)> {
    ...
    fn dependency_from_equivalences(_ecs: &EquivalentClasses<T>) -> HashSet<(Term<T>, Term<T>)> {
        ...
        let derived_dependencies = _ecs.0.iter().flat_map(|ce| Self::dependencies_from_class_expression(ce));
    ...
    fn dependencies_from_class_expression(ce0: &ClassExpression<T>) -> HashSet<(Term<T>, Term<T>)> {
        match ce0 {
            ClassExpression::Class(c) => { HashSet::new() }
            ClassExpression::ObjectIntersectionOf(ces) => { Self::dependencies_from_object_intersection_of(ce0, ces) }
            ClassExpression::ObjectUnionOf(ces) => { Self::dependencies_from_object_union_of(ce0, ces) }
            ClassExpression::ObjectComplementOf(ce) => { Self::dependencies_from_object_complement_of(ce0, ce) }
            ClassExpression::ObjectOneOf(is) => { Self::dependencies_from_object_one_of(ce0, is) }
            ClassExpression::ObjectSomeValuesFrom { ope, bce } => { Self::dependencies_from_object_some_values_from(ce0, ope, bce) }
            ClassExpression::ObjectAllValuesFrom { ope, bce } => { Self::dependencies_from_object_all_values_from(ce0, ope, bce) }
            ClassExpression::ObjectHasValue { ope, i } => { Self::dependencies_from_object_has_value(ce0, ope, i) }
            ClassExpression::ObjectHasSelf(ope) => { Self::dependencies_from_object_has_self(ce0, ope) }
            ClassExpression::ObjectMinCardinality { ope, bce, n } => { Self::dependencies_from_object_min_cardinality(ce0, ope, bce, n) }
            ClassExpression::ObjectMaxCardinality { ope, bce, n } => { Self::dependencies_from_object_max_cardinality(ce0, ope, bce, n) }
            ClassExpression::ObjectExactCardinality { ope, bce, n } => { Self::dependencies_from_object_exact_cardinality(ce0, ope, bce, n) }
            ClassExpression::DataSomeValuesFrom { dp, dr } => { Self::dependencies_from_data_some_values_from(ce0, dp, dr) }
            ClassExpression::DataAllValuesFrom { dp, dr } => { Self::dependencies_from_data_all_values_from(ce0, dp, dr) }
            ClassExpression::DataHasValue { dp, l } => { Self::dependencies_from_data_has_value(ce0, dp, l) }
            ClassExpression::DataMinCardinality { dp, dr, n } => { Self::dependencies_from_data_min_cardinality(ce0, dp, dr, n) }
            ClassExpression::DataMaxCardinality { dp, dr, n } => { Self::dependencies_from_data_max_cardinality(ce0, dp, dr, n) }
            ClassExpression::DataExactCardinality { dp, dr, n } => { Self::dependencies_from_data_exact_cardinality(ce0, dp, dr, n) }
        }
    }
    ...
    fn dependencies_from_object_complement_of<'a>(x: &'a ClassExpression<T>,ce: &'a ClassExpression<T>) -> HashSet<(Term<'a, T>, Term<'a, T>)> {
        HashSet::new()
    }

    fn dependencies_from_object_one_of<'a>(x: &'a ClassExpression<T>,is: &'a Vec<Individual<T>>) -> HashSet<(Term<'a, T>, Term<'a, T>)> {
        HashSet::new()
    }
    ...
    fn dependencies_from_object_has_value<'a>(x: &'a ClassExpression<T>,ope: &'a ObjectPropertyExpression<T>, i: &'a Individual<T>) -> HashSet<(Term<'a, T>, Term<'a, T>)> {
        HashSet::new()
    }

    fn dependencies_from_object_has_self<'a>(x: &'a ClassExpression<T>,ope: &ObjectPropertyExpression<T>) -> HashSet<(Term<'a, T>, Term<'a, T>)> {
        HashSet::new()
    }

    fn dependencies_from_object_min_cardinality<'a>(x: &'a ClassExpression<T>,ope: &'a ObjectPropertyExpression<T>, bce: &'a ClassExpression<T>, n: &'a u32) -> HashSet<(Term<'a, T>, Term<'a, T>)> {
        HashSet::new()
    }

    fn dependencies_from_object_max_cardinality<'a>(x: &'a ClassExpression<T>,ope: &'a ObjectPropertyExpression<T>, bce: &'a ClassExpression<T>, n: &'a u32) -> HashSet<(Term<'a, T>, Term<'a, T>)> {
        HashSet::new()
    }

    fn dependencies_from_object_exact_cardinality<'a>(x: &'a ClassExpression<T>,ope: &'a ObjectPropertyExpression<T>, bce: &'a ClassExpression<T>, n: &'a u32) -> HashSet<(Term<'a, T>, Term<'a, T>)> {
        HashSet::new()
    }

    fn dependencies_from_data_some_values_from<'a>(x: &'a ClassExpression<T>,dp: &'a DataProperty<T>, dr: &'a DataRange<T>) -> HashSet<(Term<'a, T>, Term<'a, T>)> {
        HashSet::new()
    }

    fn dependencies_from_data_all_values_from<'a>(x: &'a ClassExpression<T>,dp: &'a DataProperty<T>, dr: &'a DataRange<T>) -> HashSet<(Term<'a, T>, Term<'a, T>)> {
        HashSet::new()
    }

    fn dependencies_from_data_has_value<'a>(x: &'a ClassExpression<T>,dp: &'a DataProperty<T>, l: &'a Literal<T>) -> HashSet<(Term<'a, T>, Term<'a, T>)> {
        HashSet::new()
    }

    fn dependencies_from_data_min_cardinality<'a>(x: &'a ClassExpression<T>,dp: &'a DataProperty<T>, dr: &'a DataRange<T>, n: &'a u32) -> HashSet<(Term<'a, T>, Term<'a, T>)> {
        HashSet::new()
    }

    fn dependencies_from_data_max_cardinality<'a>(x: &'a ClassExpression<T>,dp: &'a DataProperty<T>, dr: &'a DataRange<T>, n: &'a u32) -> HashSet<(Term<'a, T>, Term<'a, T>)> {
        HashSet::new()
    }

    fn dependencies_from_data_exact_cardinality<'a>(x: &'a ClassExpression<T>,dp: &'a DataProperty<T>, dr: &'a DataRange<T>, n: &'a u32) -> HashSet<(Term<'a, T>, Term<'a, T>)> {
        HashSet::new()
    }
