...
pub trait SyntaxBasedDependency<T:ForIRI>: DependencyBuilder<T> {
    ...
    fn dependency_from_disjoint_classes(dcs: &DisjointClasses<T>) -> HashSet<(Term<'_, T>, Term<'_, T>)> {
        dcs.0
            .iter()
            .combinations(2)
            .flat_map(|v| {
    ...
    fn dependency_from_sub_data_property(_sdp: &SubDataPropertyOf<T>) -> HashSet<(Term<'_, T>, Term<'_, T>)> {
        HashSet::new()
    }
    ...
    fn dependency_from_data_property_domain(_dpd: &DataPropertyDomain<T>) -> HashSet<(Term<'_, T>, Term<'_, T>)> {
        HashSet::new()
    }

    fn dependency_from_data_property_range(_dpr: &DataPropertyRange<T>) -> HashSet<(Term<'_, T>, Term<'_, T>)> {
        HashSet::new()
    }
    ...
    fn dependency_from_annotation_assertion(_aa: &AnnotationAssertion<T>) -> HashSet<(Term<'_, T>, Term<'_, T>)> {
        HashSet::new()
    }
    ...
    fn dependencies_from_class_expression(ce0: &ClassExpression<T>) -> HashSet<(Term<'_, T>, Term<'_, T>)> {
        match ce0 {
