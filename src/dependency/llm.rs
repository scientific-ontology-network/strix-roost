...
pub(crate) fn ask<'a, C: Clone, T: ForIRI + 'a>(
    ...
    if depends_on.is_empty() {
        ...
        let response = client.post(request_url).body(body_text).send();
        match response {
            Ok(r) => {
                ...
                match lines.len() == depends_on.len() {
                    true => Ok(depends_on
                        .iter()
                        .zip(lines)
                        .map(|((d, c), l)| (d.clone(), (c.clone(), l)))
                        .collect()),
                    false => Err(StrixError::Error {
                        message: format!(
                            "Length of symbols does not match list returned by LLM ({}!={})",
                            lines.len(),
                            depends_on.len()
                        ),
                    }),
